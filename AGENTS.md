...
Document chargé en début de toute session Codex (CC).
...
Romain utilise les sub-agents natifs Codex. À ce jour, AUCUN
sub-agent n'est défini : le dossier `.Codex/agents/` n'existe pas
encore.
...
Quand des sub-agents seront créés dans `.Codex/agents/`, leurs rôles et
...
Pour toute modification d'interface significative dans TerrOir, CC doit
utiliser Playwright avant PR quand l'environnement le permet.
...
- exports, dashboards, tableaux, formulaires, badges, états vides,
  responsive.

Après une modification UI, CC doit :
1. lancer l'application localement ;
2. ouvrir la page concernée ;
3. vérifier desktop ;
4. vérifier mobile ;
5. tester les principaux états utilisateur ;
6. prendre au moins une capture ou décrire précisément ce qui a été
   observé ;
7. corriger les problèmes visibles avant PR.
...
- carte mal alignée ;
- état vide absent ;
- texte trop long non géré ;
- problème mobile.

Playwright ne remplace pas les tests automatisés. Il complète :
- `npm run lint` ;
- `npm run type-check` ;
- `npm run build` ;
- `npm test`.
