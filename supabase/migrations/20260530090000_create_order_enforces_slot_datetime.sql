-- =============================================================================
-- TerrOir - create_order_with_items slot datetime consistency
-- =============================================================================
-- Forward-only hardening for orders.date_retrait / orders.heure_retrait.
-- The public API now derives both values from slots.starts_at; this RPC keeps
-- the database authoritative for direct/forged calls too.
-- =============================================================================
...
begin
  ...
  select capacity_per_slot,
         ...
         (starts_at at time zone 'Europe/Paris')::time
  ...
  if p_date_retrait is distinct from v_slot_date
     or p_heure_retrait is distinct from v_slot_time then
    raise exception 'Date ou heure de retrait incoherente avec le creneau %',
                    p_slot_id
      using errcode = '23514',
            hint    = 'slot_datetime_mismatch',
            detail  = format(
              'slot_id=%s;expected_date=%s;expected_time=%s',
              p_slot_id,
              v_slot_date,
              v_slot_time
            );
  ...
  values (
    ...
    v_slot_date, v_slot_time, p_notes_client,
    v_total, 'pending'
  )
