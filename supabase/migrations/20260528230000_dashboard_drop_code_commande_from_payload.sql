-- =============================================================================
-- TerrOir — get_producer_dashboard : cleanup code_commande du payload (ADR-0015)
-- =============================================================================
-- Migration de cleanup post-deploy. À appliquer **APRÈS** le déploiement Vercel
-- du code consommateur qui lit `numero_commande` (commits de la même PR).
--
-- Avant cette migration, la RPC retournait LES DEUX champs (code_commande +
-- numero_commande) pendant la fenêtre de déploiement (cf.
-- 20260528220000_dashboard_add_numero_commande.sql). Une fois le code
-- consommateur déployé et migré sur `numero_commande`, on retire
-- `code_commande` des 3 zones :
--   - pending_orders[].code_commande
--   - upcoming_orders[].code_commande
--   - slots[].orders[].code_commande
--
-- Garantie sécurité ADR-0015 : `code_commande` (preuve de remise) ne fuite
-- plus jamais dans le payload producteur.
--
-- ⚠️ NON ADDITIF — change la SHAPE du payload. Application **après** le
-- déploiement Vercel uniquement. Cf. doctrine CLAUDE.md §8.
--
...
comment on function public.get_producer_dashboard is
  ...
  '(PPPP-CCCCC). code_commande RETIRÉ du payload (preuve de remise, ne doit pas '
  'fuiter côté producteur avant validation). Filtre orders sur (pending, confirmed). '
  'Contrat : service_role-only.';
