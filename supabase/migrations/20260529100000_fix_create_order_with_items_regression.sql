-- =============================================================================
-- TerrOir - create_order_with_items regression fix
-- =============================================================================
-- Forward-only correction after 20260528310000_create_order_blocks_unavailability.sql.
-- That migration recreated the RPC from an obsolete body:
--   - slots.actif / products.actif were referenced after both columns had been
--     renamed to active;
--   - slot capacity locking/checks, slot_full structured error and the
--     self-purchase guard were overwritten;
--   - T-434 structured 23514 hints were lost.
--
-- This migration restores the last healthy body from
-- 20260430030000_t448_p0001_wording.sql, updates the active-order count to the
-- current status model (pending, confirmed), and preserves the producer
-- unavailability guard introduced by 20260528310000.
-- =============================================================================
