...
- **T-016 Cron purge `email_change_otp_codes` rows expirées** (hors scope T-013, futur). Purge périodique des rows OTP en DB qui ne servent plus : `consumed_at IS NOT NULL` + `created_at < now() - interval '7 days'` → DELETE. Évite la croissance perpétuelle de la table. Pertinent à instrumenter avant volume significatif (négligeable aujourd'hui — quelques rows par change_email). Schedule daily ~04:00 UTC type cron Vercel. Pattern aligné `cron retry-failed-refunds` (PR #5).
- **T-018 Doc Vercel CLI workflow dans `METHODOLOGY.md`** (capitalise le setup d'aujourd'hui T-013 PR2). Vercel CLI v52.2.1 installée globalement + repo lié projet `terr-oir-21cl` (`prj_BKXoqhZcN1j3WpQ0i6HqsGj5kId7`, team `team_PPqBwWiJEXNuPUZmUUS3PIdC`) pendant la séance PR2. À documenter : install global, `vercel link`, commandes utiles (`vercel env ls/add/rm`, `vercel logs`, `vercel list`). Permet à CC de manipuler les env vars + lire les logs runtime sans passer par le Dashboard pour les futurs chantiers (workflow analogue au MCP Supabase pour DB).
