...
- **Conventions schema TerrOir à rappeler dans les briefs SQL custom** : `users.roles[]` est un text array cumulatif (`where 'consumer' = any(roles)`, pas `where role = 'consumer'`). Booléens en anglais (`active`, pas `actif`). Timestamps avec suffixe `_at`. Incident QA SQL session 27/04 matin : 2 mismatches naïfs (deviné depuis le standard Postgres au lieu du standard projet) corrigés en boucle pendant la validation. Pattern préventif : passer un snippet `information_schema.columns` au terminal CC s'il doit générer du SQL custom. Voir `LESSONS.md` section "Tests vitest".
