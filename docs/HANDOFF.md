...
> À jour le **2026-04-25 (fin de journée)** après session complète : matinée + après-midi + fin d'après-midi (chantiers Mapbox carte/markers/pin partagé, webhook decoupling, landings pro/admin, public stats + invalidation complète, layout connexion subdomain + post-login redirect cross-domain).
> Commits récents (matin/début après-midi) : `5dca301` (hover popover conseil), `22bf88e` (centralize geoloc fallback), `c3d62ee`+`83b5326`+`e03734e`+`3ea3555` (4 fixes carte Mapbox), `0761bbe`+`db63440` (webhook decoupling waitUntil), `f3fb891` (MiniMap Mapbox produit), `dcbc747`+`4b9f08d`+`ef6bfe4` (landings publiques pro/admin + middleware rewrites), `2e63dc5`+`0caf4c2`+`b07e8d8` (public stats home + revalidation cache), `6db046c` (carte WebGL markers), `ddb3a02` (Phase C.4 YAGNI clôt).
> Commits récents (fin d'après-midi) : `23d4fa0` (3D pin terra producer markers), `11b914e` (split hover layer hotfix carte), `e16459d` (couverture invalidation public-stats 1ère vague), `2652e4d` (layout /connexion adaptatif subdomain + dette redirect host-aware), `af44d64` (centralize public-stats invalidation in promote helper + 3 tests), `90bf3e3` (revalidate public-stats sur suspend/reactivate producer), `c0357f5` (revalidate public-stats sur RGPD anonymisation producer), `78e0306` (helper pin partagé /carte ↔ MiniMap), `797c89f` (helper post-login-redirect 3 niveaux d'API), `2e1a3e5` (magic link callback rôle-aware cross-domain), `8cb6114` (redirect immédiat users déjà loggés sur /connexion).
>
...
- **Redirect immédiat des users déjà loggés sur `/connexion`** (commit `8cb6114`) : check session côté server component → `redirect(canonicalPostLoginUrl(role))` immédiat. Évite l'écran de login inutile. Dette flaggée : `?redirectTo` non encore lu par `loginAction` (cf `TODO.md`).
...
- **`?redirectTo` non lu par `loginAction`** : `app/connexion/page.tsx` accepte une querystring `?redirectTo=<path>` mais `loginAction` ne la lit pas et redirige toujours sur la home rôle. À traiter pour un flow login-then-action propre. Flagué par TA dans le rapport du commit `8cb6114`.
