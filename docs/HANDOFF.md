...
> À jour le **2026-04-26 (fin de session marathon)** après suite session 25/04 → 26/04 : 4 chantiers en parallèle (auth `redirectTo`, logo SVG vectoriel + emails, vision funnel producteur Phase 1+2, audit auto-promotion + purge panier logout). 5 templates Supabase Auth Email customisés via Dashboard. Migrations `20260426000000` + `20260427000000` apply.
> Commits 25/04 (référence) : `5dca301`, `22bf88e`, `c3d62ee`+`83b5326`+`e03734e`+`3ea3555`, `0761bbe`+`db63440`, `f3fb891`, `dcbc747`+`4b9f08d`+`ef6bfe4`, `2e63dc5`+`0caf4c2`+`b07e8d8`, `6db046c`, `ddb3a02`, `23d4fa0`, `11b914e`, `e16459d`, `2652e4d`, `af44d64`, `90bf3e3`, `c0357f5`, `78e0306`, `797c89f`, `2e1a3e5`, `8cb6114`.
> Commits 25/04 fin de journée → 26/04 (cette session marathon) : `53f8f6a` (redirectTo password login), `51d409b`+`0fd3f54`+`cb3ebab`+`71905d2` (logo SVG vectoriel + 3 fixes layout + crop Inkscape), `d4088d5` (redirectTo magic link flow), `87bfff9`+`9e78ea4` (Phase 1 funnel : `source` lead + invitation directe trace), `783e071`+`a895ed2`+`49b45d8` (Phase 2 funnel : `prenom` lead + form public simplifié + wizard 2 étapes pré-rempli), `e523357` (navbar consumer agrandie xl), `4911401` (audit auto-promotion 3 conditions cumulatives), `a08a56e` (purge panier logout), `67e40fc` (logo email templates).
>
> 🔴 **Bug ouvert en fin de session** : magic link PKCE `error=auth_callback&reason=code+challenge+does+not+match`. Login mdp classique fonctionnel. Cf `TODO.md` pour plan d'investigation.
>
...
- **Supabase Dashboard > Authentication > Email Templates** : 5 templates customisés via Dashboard (Magic Link, Confirm Signup, Reset Password, Change Email, Invite User) avec header logo TerrOir sur fond crème (cohérent avec emails Resend). Pour `emailRedirectTo` passé côté server : utiliser `{{ .ConfirmationURL }}` (recommandation post-bug magic link 26/04) — Supabase compose alors l'URL avec `&` pour appender `code` à un `redirect_to` ayant déjà une query string. **Anti-pattern à éviter** : `{{ .RedirectTo }}?token_hash={{ .TokenHash }}&type=...` casse le parsing dès que `emailRedirectTo` contient `?` (concat naïve produit deux `?` dans l'URL → `URLSearchParams` ne trouve plus `token_hash`). Cf `LESSONS.md` section « Auth & sessions ».
...
- **🔴 Bug magic link PKCE — `code+challenge+does+not+match`** (ouvert 26/04 fin de session) : l'utilisateur clique le bouton « Se connecter » dans l'email et arrive sur `/connexion?error=auth_callback&reason=code+challenge+does+not+match` au lieu d'être loggé. Hypothèse : le cookie `code_verifier` posé par `signInWithOtp` côté server n'est pas retrouvé par le callback (cross-domain ou cross-context — client mail web ouvre nouveau navigateur, ou cookies cross-subdomain bloqués). Workaround : le login mdp classique reste fonctionnel. Plan d'investigation détaillé dans `TODO.md` 🔴 Bugs.
- **Phase 3 vision funnel producteur — DROP `prenom_affichage`** (reportée 26/04) : la décision 24/04 de réutiliser `users.prenom` directement pour signer le post-it « Conseil de [prenom] » nécessite ~19 fichiers transversaux (3 INSERT runtime, seed, wizard, édition onboarding, components consumer, tests). Reportée à une session dédiée pour éviter une livraison half-baked en fin de session. Cf `TODO.md` Vision funnel.
- **Renommer `StepEntreprise.tsx` → `StepInfos.tsx`** (déféré 26/04) : depuis la fusion `StepPersonnel` + `StepEntreprise` (commit `49b45d8`), le composant gère désormais perso ET entreprise. Le nom n'est plus aligné. Renommage cosmétique trivial mais déféré pour ne pas mélanger refactor et delivery dans le commit `49b45d8`.
- **Tests unitaires `isValidRedirectPath` + `resolvePostLoginPath`** (déféré 26/04) : helpers ajoutés à `lib/auth/post-login-redirect.ts` aux commits `53f8f6a` + `d4088d5`, pas encore couverts par vitest. Critiques pour la sécurité (anti open-redirect) — à ajouter avant lancement public.
- **UI `/producer-interests` n'affiche pas la colonne `source`** (Phase 2bis) : la colonne DB existe (commit `87bfff9`) et est alimentée correctement, mais le `LeadsTable` admin ne la montre pas encore. Petit chantier UX pour distinguer `formulaire_public` vs `invitation_directe` dans l'onglet Leads.
...
- **SMTP custom Supabase à confirmer** : la doc HANDOFF mentionne le custom SMTP Resend configuré (commit `dbe6360` 23/04). Observation récente : mails Auth Supabase atterrissant en spam (à recouper avec le bug magic link PKCE — peut-être que les 2 sont liés à un problème SMTP). À vérifier dans Supabase Dashboard > Auth > SMTP que la config Resend est toujours active et la clé valide.
