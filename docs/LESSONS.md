...
- **Magic link PKCE — `code_verifier` cookie cross-context** (bug ouvert 26/04, plan d'investigation dans `TODO.md`) : le flow PKCE de Supabase pose un cookie `code_verifier` côté server au moment du `signInWithOtp`, lu par le callback `?code=` lors du clic sur l'email. Si l'email est ouvert dans un autre browser/contexte (client mail web qui lance un autre navigateur, mobile vs desktop), le cookie n'est pas accessible → erreur `code challenge does not match`. À investiguer + envisager le flow OTP `?token_hash=&type=` (`verifyOtp`) qui n'utilise pas de cookie côté server.
