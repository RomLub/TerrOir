...
- ⚠️ **Tracking `trackedUserIds` / `trackedRowIds` séparés** (résolu T-021) : historiquement le Set `ctx.trackedIds` contenait à la fois les UUIDs user ET les UUIDs de rows OTP créées par `seedOtp`, ce qui produisait des warnings "User not found" quand `cleanupAllTrackedUsers` itérait dessus. Depuis T-021, les UUIDs sont séparés en deux Sets : `trackedUserIds` (cleanup via `auth.admin.deleteUser`) et `trackedRowIds` (cleanup via cascade FK `ON DELETE CASCADE` depuis l'user parent). La validation `safeUpdate`/`safeDelete` accepte un id présent dans l'**union** des deux Sets, donc les helpers existants restent valides sans changement côté specs E2E.
