...
function PanierClientInner() {
  ...
  const orderGroups = useMemo(() => groupCartItems(items), [items]);
  const hasMultipleGroups = orderGroups.length > 1;
  ...
  return (
    <>
      <p className="text-[14px] text-dark/60 mt-1">
        {items.length} article{items.length > 1 ? 's' : ''} dans {orderGroups.length} commande{orderGroups.length > 1 ? 's' : ''} distincte{orderGroups.length > 1 ? 's' : ''}
      </p>
      ...
      {hasMultipleGroups && (
        <div className="mt-6 rounded-xl border border-terra-300/40 bg-terra-100/60 p-4 text-[13px] leading-relaxed text-terra-900">
          Votre panier contient plusieurs commandes distinctes. Chaque producteur et créneau nécessite une commande séparée.
        </div>
      )}
      ...
      <div className="mt-10 grid lg:grid-cols-[1fr_380px] gap-10 items-start">
        <div className="space-y-6">
          {orderGroups.map((group) => {
            ...
            return (
              <section key={group.id} className="bg-white rounded-2xl border border-dark/[0.06] shadow-soft overflow-hidden">
                <header className="px-5 py-4 border-b border-dark/[0.06] flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="text-[11px] uppercase tracking-[0.14em] text-terra-700 font-semibold">Commande chez</div>
                    <Link href={`/producteurs/${group.slug}`} className="font-serif text-[22px] text-green-900 hover:text-green-700">{group.producerName}</Link>
                    <div className="mt-1 text-[12px] text-dark/60">Retrait · {formatDateFr(group.dateRetrait)}</div>
                  </div>
                  <div className="flex flex-col items-start gap-3 sm:items-end">
                    <Badge>{group.items.length} article{group.items.length > 1 ? 's' : ''}</Badge>
                    {hasMultipleGroups && (
                      <Link href={`/compte/checkout?group=${encodeURIComponent(group.id)}`} className="w-full sm:w-auto">
                        <Button size="sm" className="w-full sm:w-auto">Commander cette commande</Button>
                      </Link>
                    )}
                  </div>
                </header>
                ...
                <footer className="flex items-center justify-between gap-4 border-t border-dark/[0.06] bg-green-100/40 px-5 py-4">
                  <span className="text-[13px] text-dark/65">Total de cette commande</span>
                  <span className="font-serif text-[22px] text-green-900 tabular-nums">{groupTotal.toFixed(2).replace('.', ',')} €</span>
                </footer>
              </section>
            );
          })}
        ...
        <aside className="lg:sticky lg:top-24 bg-white rounded-2xl border border-dark/[0.06] shadow-soft p-6">
          ...
          {hasMultipleGroups ? (
            <p className="mt-6 rounded-xl bg-green-100/60 p-3 text-center text-[12px] leading-relaxed text-dark/70">
              Lance le paiement depuis la commande que tu veux régler.
            </p>
          ) : (
            <Link href={`/compte/checkout?group=${encodeURIComponent(orderGroups[0]?.id ?? '')}`}>
              <Button size="lg" className="w-full mt-6">Commander →</Button>
            </Link>
          )}
