...
// revival_blocked_* (vs 1 générique) : permet l'analytics fine "combien de
// blocages stock vs slot ce mois-ci".
type DisplayStatus =
  | Status
...
export type SuiviCommandesClientProps = {
  initialOrders: AdminOrder[];
  initialError: string | null;
};

export function SuiviCommandesClient({ initialOrders, initialError }: SuiviCommandesClientProps) {
  ...
  const metrics = useMemo(() => {
    ...
    // Les void orders (payment_failed, revival_blocked_*) ne sont pas de
    // vraies commandes (paiement non finalisé OU refundé automatiquement
  ...
  return (
    <div>
      <AdminPageHeader
        ...
        subtitle={`${orders.length} commandes récentes`}
        error={initialError}
        right={
